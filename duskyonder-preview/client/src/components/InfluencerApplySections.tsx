...
export function FormField({
  ...
  field: InfluencerConfig["formFields"][0];
}) {
  ...
  if (field.type === "text" || field.type === "email") {
    return (
      <div className="inf-form-field">
        <label className="inf-form-label">
          {field.label}
          {field.required && <span className="inf-form-required">*</span>}
        </label>
        <input
          type={field.type}
          className="inf-form-input"
...
export function BenefitsSection({ cfg }: { cfg: InfluencerConfig }) {
  ...
  return (
    <section
      className={`inf-section inf-benefits-section inf-benefits-section--minimal${vis ? " inf-animate-in" : ""}`}
      ref={ref}
    >
      <div className="inf-section-inner">
        {isVisible("benefitsTitle") && (
          <h2
            className="inf-section-title inf-apply-section-title"
            style={ts("benefitsTitle")}
          >
            {cfg.benefitsTitle}
          </h2>
        )}
        {isVisible("benefitsSubtitle") && (
          <p className="inf-section-subtitle" style={ts("benefitsSubtitle")}>
            {cfg.benefitsSubtitle}
          </p>
        )}
            ...
            .map((b, i) => (
              ...
              >
                ...
                {isVisible(`benefitTitle_${b.id}`) && (
                  <div
                    className="inf-benefit-title"
                    style={ts(`benefitTitle_${b.id}`)}
                  >
                    {b.title}
                  </div>
                )}
                {isVisible(`benefitDesc_${b.id}`) && (
                  <div
                    className="inf-benefit-desc"
                    style={ts(`benefitDesc_${b.id}`)}
                  >
                    {b.description}
                  </div>
                )}
...
export function RequirementsSection({ cfg }: { cfg: InfluencerConfig }) {
  ...
  return (
    ...
    >
      <div
        className="inf-section-inner"
        style={{ maxWidth: cfg.reqSectionMaxWidth ?? 1100 }}
      >
        {isVisible("requirementsTitle") && (
          <h2
            className="inf-section-title inf-apply-section-title"
            style={ts("requirementsTitle")}
          >
            {cfg.requirementsTitle}
          </h2>
        )}
        <div className="inf-req-grid">
          <div className="inf-req-col">
            {isVisible("requirementsLeftTitle") && (
              <h3
                className="inf-req-col-title"
                style={ts("requirementsLeftTitle")}
              >
                {cfg.requirementsLeftTitle}
              </h3>
            )}
            <ul className="inf-req-list">
              {(cfg.requirementsLeft ?? [])
          ...
          <div className="inf-req-col">
            {isVisible("requirementsRightTitle") && (
              <h3
                className="inf-req-col-title"
                style={ts("requirementsRightTitle")}
              >
                {cfg.requirementsRightTitle}
              </h3>
            )}
            <ul className="inf-req-list">
              {(cfg.requirementsRight ?? [])
                .filter(r => r.visible !== false)
                .map(r => (
                  <li key={r.id} className="inf-req-item">
                    <span className="inf-req-check">✓</span>
...
export function FormSection({ cfg }: { cfg: InfluencerConfig }) {
  ...
  return (
    <section
      className={`inf-section inf-form-section inf-form-section--minimal${vis ? " inf-animate-in" : ""}`}
      id="apply"
      ref={ref}
    >
      <div className="inf-section-inner">
        ...
        {isVisible("formSubtitle") && (
          <p className="inf-section-subtitle" style={ts("formSubtitle")}>
            {cfg.formSubtitle}
          </p>
        )}
        {formSubmitted ? (
          <div className="inf-form-success">
            ...
            <p>
              Thank you for applying. We'll review your profile and get back to
...
export function FaqSection({ cfg }: { cfg: InfluencerConfig }) {
  ...
  return (
    <section
      className={`inf-section inf-faq-section inf-faq-section--minimal${vis ? " inf-animate-in" : ""}`}
      ref={ref}
    >
...
export function FooterCtaSection({ cfg }: { cfg: InfluencerConfig }) {
  ...
  return (
    <section
      className="inf-footer-cta"
      style={{ background: cfg.heroBgColor ?? "#0D3D2B" }}
    >
      <div className="inf-footer-cta-inner">
        {isVisible("footerCtaTitle") && (
          <h2 className="inf-footer-cta-title" style={ts("footerCtaTitle")}>
            {cfg.footerCtaTitle}
          </h2>
        )}
        {isVisible("footerCtaSubtitle") && (
          <p
            className="inf-footer-cta-subtitle"
            style={ts("footerCtaSubtitle")}
          >
            {cfg.footerCtaSubtitle}
          </p>
        )}
        <div className="inf-footer-cta-btns">
          {isVisible("footerCtaPrimaryBtn") && (
            <a
              href={cfg.footerCtaPrimaryLink ?? "#"}
              className="inf-footer-cta-btn-primary"
              style={ts("footerCtaPrimaryBtn")}
            >
          ...
          {isVisible("footerCtaSecondaryBtn") && (
            <a
              href={cfg.footerCtaSecondaryLink ?? "/pages/blog"}
              className="inf-footer-cta-btn-secondary"
              style={ts("footerCtaSecondaryBtn")}
            >
...
export const SECTION_MAP: Record<
  InfluencerApplySectionKey,
  React.ComponentType<{ cfg: InfluencerConfig }>
> = {
