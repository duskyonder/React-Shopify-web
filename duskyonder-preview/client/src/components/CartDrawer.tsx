...
export function CartDrawer() {
  ...
  const freeShippingText =
    config.freeShippingText ?? "Spend {{amount}} more for free shipping";
  const freeShippingAchievedText =
    config.freeShippingAchievedText ?? "You've unlocked free shipping! 🎉";

  const subtotal = shopifySubtotal
    ? parseFloat(shopifySubtotal)
  ...
  const currencySymbol =
    items[0]?.price?.replace(/[\d.,\s]/g, "").trim() || "$";
  ...
  const recommendedProducts = useMemo(() => {
    ...
    if (related.length < 4) {
      const fallback = allProducts.filter(
        p => !cartIds.has(p.id) && !seen.has(p.id)
      );
      ...
      >
        ...
        {items.length > 0 && (
          <FreeShippingBar
            ...
            achievedText={freeShippingAchievedText}
          />
        )}
          ...
          {items.length === 0 ? (
            ...
            >
              ...
              <button
                ...
                onMouseEnter={e =>
                  (e.currentTarget.style.background = BTN_CHECKOUT_HOVER)
                }
                onMouseLeave={e =>
                  (e.currentTarget.style.background = BTN_CHECKOUT_BG)
                }
              >
                Continue Shopping
              </button>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          style={{
                            fontSize: cartItemPriceFontSize,
                            fontWeight: 600,
                            color: TEXT_PRIMARY,
                            flexShrink: 0,
                          }}
