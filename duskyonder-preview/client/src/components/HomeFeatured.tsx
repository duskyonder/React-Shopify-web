...
function DesktopProductPager({
  products,
  productsPerRow,
  renderCard,
  gap = 0,
  cardWidth: fixedCardWidth = 0,
  maxWidth = 1680,
  cardHeight = 380,
}: {
  products: Product[];
        ...
        style={
          {
            ...
            ["--product-card-height" as string]:
              cardHeight > 0 ? `${cardHeight}px` : "auto",
        ...
        >
          ...
          >
            {Array.from({ length: totalPages }).map((_, pi) =>
              products
                .slice(pi * productsPerRow, (pi + 1) * productsPerRow)
                .map(product => (
                  <div
                    key={product.id}
                    style={{
      ...
      {totalPages > 1 && (
        <div className="sf-section-dots">
          {Array.from({ length: totalPages }).map((_, i) => (
            <button
              key={i}
              className={`sf-section-dot${i === page ? " active" : ""}`}
              onClick={() => setPage(i)}
            />
          ))}
        </div>
...
function SFFeatured({
  ...
  const [hoveredColors, setHoveredColors] = useState<Record<string, number>>(
    {}
  ); // productId → colorIdx
  ...
  const instanceData = isDefault
    ? null
    : config.featuredInstances?.find(f => f.id === instanceId);

  // Determine data source
  const dataSource = isDefault
    ? (config.productsDataSource ?? "manual")
    : (instanceData?.dataSource ?? "manual");
  const collectionHandle = isDefault
    ? (config.productsCollectionHandle ?? "")
    : (instanceData?.collectionHandle ?? "");
  ...
  const productsPerRow = isDefault
    ? (config.productsPerRow ?? 4)
    : (instanceData?.productsPerRow ?? 4);
  const productAspectRatio = isDefault
    ? (config.productAspectRatio ?? "3/4")
    : (instanceData?.productAspectRatio ?? "3/4");
    ...
    return (
      ...
      >
        ...
        <div
          ...
          style={{ aspectRatio: productAspectRatio }}
        >
          <div className={"sf-product-img-primary"}>
            {displayImg ? (
              <img loading="lazy" src={displayImg} alt={product.name} />
            ) : (
              <ImgPlaceholder label="产品图片" />
            )}
          </div>
        ...
        <div className="sf-product-info">
          ...
          <div
            ...
            style={{
              position: "relative",
              zIndex: 10,
              gap: `${config.productsSwatchGap ?? 4}px`,
              marginLeft: config.productsSwatchOffsetX
                ? `${config.productsSwatchOffsetX}px`
                : undefined,
              marginTop: `${config.productsSwatchMarginTop ?? 6}px`,
              justifyContent: config.productsSwatchAlign ?? "flex-start",
            }}
          >
            {product.colors.map((color, i) => (
              <div
                ...
                onClick={e => {
                  e.preventDefault();
                  e.stopPropagation();
                  setHoveredColors(prev => ({ ...prev, [product.id]: i }));
                }}
              >
                <ColorSwatch
                  value={color}
  ...
  return (
    <>
      <section
        className="sf-section sf-featured"
        style={
          {
      ...
      >
        <div
          className="sf-section-header sf-featured-header"
        ...
        {isMobile ? (
          ...
          <div
            className="sf-products-mobile-outer"
            style={
              {
                ...(config.productsMobileMaxWidth
                  ? {
                      maxWidth: `${config.productsMobileMaxWidth}px`,
                      margin: "0 auto",
                    }
                  : {}),
                ["--product-card-height" as string]:
                  config.productsMobileCardHeight
                    ? `${config.productsMobileCardHeight}px`
                    : undefined,
              } as React.CSSProperties
            }
          >
              ...
              {products.map((product, idx) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      scrollSnapAlign:
                        idx % mobileCardCount === 0 ? "start" : "none",
                      flexShrink: 0,
                      flex: `0 0 ${mobileW}`,
                      width: mobileW,
                    }}
            ...
            {mobileTotalPages > 1 && (
              <div className="sf-section-dots">
                {Array.from({ length: mobileTotalPages }).map((_, i) => (
                  <button
                    key={i}
                    className={`sf-section-dot${i === mobilePage ? " active" : ""}`}
