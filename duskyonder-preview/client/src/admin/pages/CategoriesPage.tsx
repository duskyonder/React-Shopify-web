...
export default function CategoriesPage() {
  const { config, updateConfig, updateCategory, addCategory, removeCategory } =
    useThemeConfig();
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e =>
                  updateConfig({ categoriesTitle: e.target.value })
                }
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e =>
                  updateConfig({ categoryAspectRatio: e.target.value })
                }
                placeholder="3/4"
                className="h-8"
              />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label className="text-xs">
                桌面端列间距（{config.categoriesDesktopGap ?? 12}px）
              </Label>
              <Slider
                value={[config.categoriesDesktopGap ?? 12]}
                onValueChange={([v]) =>
                  updateConfig({ categoriesDesktopGap: v })
                }
                min={0}
                max={48}
                step={2}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">
                移动端列间距（{config.categoriesMobileGap ?? 8}px）
              </Label>
              <Slider
                value={[config.categoriesMobileGap ?? 8]}
                onValueChange={([v]) =>
                  updateConfig({ categoriesMobileGap: v })
                }
                min={0}
                max={32}
                step={2}
            ...
            <CardContent className="pt-4 pb-4 space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={cat.link}
                  onChange={e =>
                    updateCategory(cat.id, { link: e.target.value })
                  }
